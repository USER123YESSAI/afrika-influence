...
export default function AdminSignalementsPage() {
  ...
  const load = async (statut = filterStatut) => {
    setLoading(true);
    try {
      const d: any = await adminApi.getSignalements(statut ? { statut } : undefined);
  ...
  const handleAction = async (statut: string) => {
    ...
    setProc(modal.id);
    try {
      await adminApi.traiterSignalement(modal.id, { statut, decisionAdmin });
      showToast(statut === 'TRAITE' ? '✅ Signalement traité' : '❌ Signalement rejeté');
  ...
  return (
    <DashboardAdmin title="Signalements" subtitle="Gérer les rapports des utilisateurs">
      {toast && (
          ...
          <div className="bg-white rounded-3xl shadow-bento p-6 w-full max-w-lg space-y-4">
            ...
            <div className="flex gap-2">
              <button onClick={() => handleAction('TRAITE')} disabled={!!processing}
                className="flex-1 py-2.5 bg-gradient-brand text-white text-sm font-semibold rounded-xl hover:opacity-90 disabled:opacity-50 transition-all hover-lift">
                ✅ Marquer Traité
              </button>
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-3xl border border-gray-100 shadow-bento p-6">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-lg font-semibold text-gray-900">Liste des signalements (Vue globale)</h2>
                ...
                <div key={s.id}
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {s.statut === 'TRAITE' && <span className="text-xs px-2 py-0.5 rounded-full bg-brand-100 text-brand-700">Traité</span>}
                    </div>
